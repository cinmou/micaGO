...
在依赖它之前,请先阅读 [安全模型](#-安全模型) 与 [局限性](#-局限性)。与 Apple 无任何关联。
...
- 🔔 **通知(可选)。** 一个后台保活服务就能发出原生 Android MessagingStyle 通知,什么都
  不用配。想用推送,改接你 **自己的** Firebase 也行。
...
简单说,MicaGo 只把你的 iMessage 通过你掌控的连接桥接到你的设备;它不运行云服务、不持有账号、
不上传通讯录,也不会把消息存到你的 Mac 以外。
...
- 🔋 **保活服务(安卓)。** 最简单、多数人会用的方式:一个前台服务把连接保持打开,来消息时
  弹出本地通知 —— 不需要推送账号,也不需要 `google-services.json`。默认关闭;厂商电池策略
  仍可能限制它。
- 🔔 **Firebase / FCM 推送。** 不想一直挂着服务?那就接上你 **自己的** Firebase 项目走后台
  推送(什么都不内置)。负载只是一个唤醒信号,消息本身通过 WebSocket 或增量同步到达。参见
...
- **应用被杀后的通知** 靠保活服务;你愿意用 Firebase 的话,也可以靠你自己的
  `google-services.json`。两者都没有时,通知只能尽力而为,重新打开后由 socket 加增量同步把
  消息补齐。

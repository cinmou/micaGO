...
- **Windows UI:** low-tint `MicaKind.Base` is intentionally visible through the native title bar and sidebar; the contact bar belongs inside the rounded conversation surface and the message canvas stays opaque (`MicaGoChatSurfaceBrush`). Native caption buttons require `TitleBarHeightOption.Standard` plus transparent theme-aware button colors. Window sizing is Per-Monitor V2 and converts logical design dimensions with `XamlRoot.RasterizationScale`, then centers inside the display work area. Message direction/corner transitions use XAML visual states; do not reintroduce hard-coded light/dark colors in code-behind.

...
这些指南帮助你配置 Mac 应用、从手机连接,并(可选)从任何地方访问你的 Mac。
micaGO 让你 **自己的** 设备与 **你自己的** Mac 通信,消息在你的 Mac 和你连接的设备之间流转。
...
- **[使用 Cloudflare Tunnel 远程访问](remote-access-cloudflare.md)**(英文)—— 借助
  你自己的域名从家庭网络外访问你的 Mac。隧道是 **外部且可选** 的,由你管理。
...
- **远程访问** 使用你自己的域名 + Cloudflare Tunnel,或你选择并管理的其他隧道。
